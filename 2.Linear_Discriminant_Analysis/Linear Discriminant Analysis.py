...
def lda_dimensionality_reduction(indep_X, dep_Y, n):
    # Scaling the data
    scaler = StandardScaler()
    indep_X_scaled = scaler.fit_transform(indep_X)

    # LDA apply panni top 'n' components edukkum
    lda = LDA(n_components=n)
    selected_features = lda.fit_transform(indep_X_scaled, dep_Y)

    # Explained Variance return panna LDA-la support illa, so null return pannu
...
selected_features, explained_variance = lda_dimensionality_reduction(indep_X, dep_Y, 1)
